...
hal_err_t dma_clear_flags(DMA_TypeDef* handle, uint8_t stream);

void dma_enable_stream(DMA_Stream_TypeDef* stream);
hal_err_t dma_disable_stream(DMA_Stream_TypeDef* stream);

void dma_enable_circular_mode(DMA_Stream_TypeDef* stream);
...
void dma_set_addresses(DMA_Stream_TypeDef* stream, uint32_t* p, uint32_t* m1, uint32_t* m2);
