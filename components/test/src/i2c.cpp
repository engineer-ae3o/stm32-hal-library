    ...
    namespace {
        ...
        void invalid_arg_guards() {
            ...
            TEST_ASSERT_EQUAL(HAL_ERR_INVALID_ARG,
                               i2c_master_transceive(nullptr, 0x50, data.data(), data.size(), rx_buf.data(), rx_buf.size()));
            TEST_ASSERT_EQUAL(HAL_ERR_INVALID_ARG,
                               i2c_master_transceive(I2C1, 0, data.data(), data.size(), rx_buf.data(), rx_buf.size()));
            TEST_ASSERT_EQUAL(HAL_ERR_INVALID_ARG,
                               i2c_master_transceive(I2C1, 0x80, data.data(), data.size(), rx_buf.data(), rx_buf.size()));
            TEST_ASSERT_EQUAL(HAL_ERR_INVALID_ARG,
                               i2c_master_transceive(I2C1, 0x50, nullptr, data.size(), rx_buf.data(), rx_buf.size()));
            TEST_ASSERT_EQUAL(HAL_ERR_INVALID_ARG, i2c_master_transceive(I2C1, 0x50, data.data(), 0, rx_buf.data(), rx_buf.size()));
            TEST_ASSERT_EQUAL(HAL_ERR_INVALID_ARG,
                               i2c_master_transceive(I2C1, 0x50, data.data(), data.size(), nullptr, rx_buf.size()));
