...
hal_err_t adc_injected_group_start_conv(ADC_TypeDef* handle, const adc_injected_group_config_t* config) {
    ...
    switch (config->channels.num_of_channels) {
        case 1:
            ...
            handle->JOFR4 |= (uint32_t)(config->offsets[0] << ADC_JOFR4_JOFFSET4_Pos) & ADC_JOFR4_JOFFSET4;
            break;
        case 2:
            ...
            handle->JOFR3 |= (uint32_t)(config->offsets[0] << ADC_JOFR3_JOFFSET3_Pos) & ADC_JOFR3_JOFFSET3;
            handle->JOFR4 |= (uint32_t)(config->offsets[1] << ADC_JOFR4_JOFFSET4_Pos) & ADC_JOFR4_JOFFSET4;
            break;
        case 3:
            ...
            handle->JOFR2 |= (uint32_t)(config->offsets[0] << ADC_JOFR2_JOFFSET2_Pos) & ADC_JOFR2_JOFFSET2;
            handle->JOFR3 |= (uint32_t)(config->offsets[1] << ADC_JOFR3_JOFFSET3_Pos) & ADC_JOFR3_JOFFSET3;
            handle->JOFR4 |= (uint32_t)(config->offsets[2] << ADC_JOFR4_JOFFSET4_Pos) & ADC_JOFR4_JOFFSET4;
            break;
        case 4:
...
hal_err_t adc_injected_group_get_result(ADC_TypeDef* handle, uint16_t* raw_data_buffer, size_t buffer_size) {
    ...
    switch (buffer_size) {
        case 1:
            raw_data_buffer[0] = (uint16_t)handle->JDR4;
            break;
        case 2:
            raw_data_buffer[0] = (uint16_t)handle->JDR3;
            raw_data_buffer[1] = (uint16_t)handle->JDR4;
            break;
        case 3:
            raw_data_buffer[0] = (uint16_t)handle->JDR2;
            raw_data_buffer[1] = (uint16_t)handle->JDR3;
            raw_data_buffer[2] = (uint16_t)handle->JDR4;
            break;
        case 4:
